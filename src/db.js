...
function initDb() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS earn_sessions (
      ...
      provider TEXT NOT NULL DEFAULT 'lootlabs',
  ...
  const addColumn = (name, sql) => {
    if (!sessionColumns.includes(name)) {
      db.exec(`ALTER TABLE earn_sessions ADD COLUMN ${sql}`);
    }
  };

  addColumn('provider', `provider TEXT NOT NULL DEFAULT 'lootlabs'`);
  addColumn('reward_amount', `reward_amount INTEGER NOT NULL DEFAULT 100`);
  addColumn('lootlabs_url', 'lootlabs_url TEXT');
  addColumn('lootlabs_short', 'lootlabs_short TEXT');
  addColumn('linkvertise_url', 'linkvertise_url TEXT');
...
function getRewardForProvider(provider) {
  return provider === 'linkvertise'
    ? config.linkvertiseRewardAmount
    : config.lootlabsRewardAmount;
}

function createEarnSession(discordId, provider = 'lootlabs') {
  const sessionId = makeId(20);
  const t = now();
  const expires = t + config.sessionExpiryMinutes * 60 * 1000;
  const rewardAmount = getRewardForProvider(provider);

  db.prepare(`
    INSERT INTO earn_sessions(session_id, discord_id, provider, reward_amount, status, created_at, expires_at)
    VALUES(?, ?, ?, ?, 'pending', ?, ?)
  `).run(sessionId, discordId, provider, rewardAmount, t, expires);
...
function getActiveSession(discordId, provider = null) {
  if (provider) {
    return db.prepare(`
...
function updateSessionLootStarted(sessionId, token, lootlabsUrl, lootlabsShort = null, ipHint = null) {
  db.prepare(`
    ...
    SET status = 'loot_started',
        token = ?,
        lootlabs_url = ?,
        lootlabs_short = ?,
        ip_hint = COALESCE(?, ip_hint)
    WHERE session_id = ?
...
function updateSessionLinkvertiseStarted(sessionId, token, linkvertiseUrl, ipHint = null) {
  db.prepare(`
    UPDATE earn_sessions
    SET status = 'linkvertise_started',
        token = ?,
        linkvertise_url = ?,
        ip_hint = COALESCE(?, ip_hint)
    WHERE session_id = ?
  `).run(token, linkvertiseUrl, ipHint, sessionId);
}
...
function completeSessionFromPostback(sessionId, uniqueId, ip = null, rawQuery = '') {
  ...
  const tx = db.transaction(() => {
    ...
    if (session.provider !== 'lootlabs') return { ok: false, code: 'WRONG_PROVIDER', session };
    ...
    if (session.status !== 'loot_started') {
      return { ok: false, code: 'SESSION_NOT_READY', session };
    }
...
function completeLinkvertiseSession(sessionId, token) {
  const t = now();
  ...
  const tx = db.transaction(() => {
    ...
    if (!session) return { ok: false, code: 'SESSION_NOT_FOUND' };
    if (session.provider !== 'linkvertise') return { ok: false, code: 'WRONG_PROVIDER', session };
    if (session.status === 'completed') return { ok: false, code: 'ALREADY_COMPLETED', session };
    if (session.token !== token) return { ok: false, code: 'BAD_TOKEN', session };
    ...
    if (session.status !== 'linkvertise_started') {
      return { ok: false, code: 'SESSION_NOT_READY', session };
    }
...
function createRedemption(discordId, amount, item) {
  ...
  if (user.balance < amount) {
    return { ok: false, error: 'INSUFFICIENT_BALANCE', user };
  }
...
module.exports = {
  ...
  updateSessionLinkvertiseStarted,
  completeSessionFromPostback,
  completeLinkvertiseSession,
